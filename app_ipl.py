...
def remove(username, pw):
    if(username != 'admin'):
        ...
        cur.execute(
            "select column_name from information_schema.columns where table_schema = 'public' and table_name='sponsors'")
        col = [row[0] for row in cur]
        #venue = st.text_input("Venue id")
        #tup = (df['venue_id'])
        to_delete = st.text_input(
            "Enter the name of the sponsor to be deleted")
        click = st.button('Delete', key=1)
...
def update(username, pw):
    if(username != 'admin'):
        ...
        cur.execute(
            "select column_name from information_schema.columns where table_schema = 'public' and table_name='venue'")
        col = [row[0] for row in cur]
        #venue = st.text_input("Venue id")
        #tup = (df['venue_id'])
        venue = st.selectbox("Choose venue", options=(
            'V1', 'V2', 'V3', 'V4', 'V5', 'V6'))
...
with st.sidebar.form(key='loginform', clear_on_submit=False):
    option = st.sidebar.selectbox('Choose user', options=(
        '', 'staff_1a', 'staff_1b', 'admin', 'viewer'), key='username')
    passw = st.sidebar.text_input(
        'password', placeholder=f'enter password for {option}', type='password')
    st.sidebar.button('sign in', key='login')
ref = st.container()
login(option, passw)
page = st.sidebar.selectbox('Choose page', options=(
    'read', 'delete', 'update', 'simple queries', 'complex queries'))
with ref:
    if passw == '' or option == '':
